--[[ BetterTogether API Dump (dev-only)
  A throwaway tool that snapshots the *live* client API so our static analysis matches
  the exact game build, instead of a hand-maintained guess. NOT shipped with the addon.

  Usage in-game:
    1. Enable "BetterTogether API Dump" at the character-select AddOns screen.
    ...
    4. Back in the repo: pwsh tools/gen-wow-api.ps1   (luacheck globals)
                         pwsh tools/gen-luals-defs.ps1 (LuaLS signatures)

  Two products land in SavedVariables (BetterTogetherAPIDumpDB):
...
SlashCmdList["BTAPIDUMP"] = function()
  ...
  BetterTogetherAPIDumpDB = {
    build = tostring(major) .. " interface " .. tostring(interface),
    globalCount = select(2, globals:gsub("\n", "\n")) + 1,
    globals = globals,
    emmylua = emmy,
  }
  print("|cff00ff00BTAPIDump|r captured "
    .. BetterTogetherAPIDumpDB.globalCount .. " globals + "
    .. select(2, emmy:gsub("\nfunction ", "")) .. " documented functions for build "
    .. BetterTogetherAPIDumpDB.build .. ".")
